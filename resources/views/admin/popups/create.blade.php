            ...
            <form action="{{ route('admin.popups.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <select name="format" id="format" class="form-control @error('format') is-invalid @enderror" required>
                        <option value="1" {{ old('format', '1') == '1' ? 'selected' : '' }}>Format 1 - Full (Image, Icon, Title, Subtitle, Message)</option>
                        <option value="2" {{ old('format') == '2' ? 'selected' : '' }}>Format 2 - Simple (Image, Title)</option>
                    </select>
                    @error('format')
                ...
                <div class="form-group format-1-fields">
                    ...
                    <input type="text" name="subtitle" id="subtitle" class="form-control @error('subtitle') is-invalid @enderror" 
                           value="{{ old('subtitle') }}" placeholder="ঢাকা–১৩ আসনের সংসদ সদস্য পদপ্রার্থী">
                    @error('subtitle')
                        <span class="invalid-feedback">{{ $message }}</span>
                    @enderror
                ...
                <div class="form-group format-1-fields">
                    ...
                    <textarea name="message" id="message" rows="3" class="form-control @error('message') is-invalid @enderror">{{ old('message') }}</textarea>
                    @error('message')
                        <span class="invalid-feedback">{{ $message }}</span>
                    @enderror
                </div>
